...
function renderPasswords(passwords, total) {
  ...
  for (const letter of letters) {
    ...
    html += `<div class="category-group">
      <div class="category-title cat-pwd"><span class="cat-dot"></span>${escapeHtml(letter)} (${items.length})</div>
...
function renderProfiles(profiles, total) {
  ...
  profilesArea.innerHTML = `<div class="category-group">
    <div class="category-title cat-profile"><span class="cat-dot"></span>Profiles (${total})</div>
    <table class="wallet-table">
...
function renderCards(cards, total) {
  ...
  cardsArea.innerHTML = `<div class="category-group">
    <div class="category-title cat-card"><span class="cat-dot"></span>Credit Cards (${total})</div>
    <table class="wallet-table">
...
function renderResults(entries, total) {
  ...
  if (total === 0) {
    resultsArea.innerHTML = `<p class="no-wallets">&#x2714; No browsing history found.</p>`;
    return;
  }
  ...
  for (const letter of letters) {
    ...
    html += `<div class="category-group">
      <div class="category-title cat-letter"><span class="cat-dot"></span>${escapeHtml(letter)} (${items.length})</div>
      <table class="wallet-table">
...
function handleIncomingEvent(event, payload) {
  if (event === "ready") {
    ...
    log(`Scan complete at ${scannedAt} \u2014 ${total} URL(s) \u00B7 ${pwdTotal} pwd \u00B7 ${profileTotal} profiles \u00B7 ${cardTotal} cards`);
...
if (!clientId) {
  log("No clientId provided in URL \u2014 open this plugin from a client\u2019s context menu");
