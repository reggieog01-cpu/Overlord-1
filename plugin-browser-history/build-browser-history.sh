#!/usr/bin/env bash
# Build script for the browser-history Overlord plugin.
# Produces Windows binaries only. Requires x86_64-w64-mingw32-gcc for cross-compilation.
#
# Usage:
#   ./build-browser-history.sh
#   BUILD_TARGETS="windows-amd64 windows-arm64" ./build-browser-history.sh
set -euo pipefail
...
for target in ${BUILD_TARGETS}; do
  ...
  GOWORK=off GOTOOLCHAIN=local CGO_ENABLED=1 \
    ...
    /usr/local/go/bin/go build -buildmode=c-shared -o "${outfile}" .
